...
from fd_para import *
from fd_training import *
from fd_plotting import *

workdir = os.path.dirname(os.getcwd())
model = 'ah_0130_vt_on_v_off_4d'
srcdir = workdir + '/src/'
...
V                        = np.genfromtxt(mfrSuite_resdir + '/V.dat')
Vtilde                   = np.genfromtxt(mfrSuite_resdir + '/Vtilde.dat')
X                        = np.array([W,Z,V,Vtilde]).transpose()
...
variables = calc_var(logXiH_NN,logXiE_NN, kappa_NN, W.reshape(-1,1),Z.reshape(-1,1),V.reshape(-1,1), Vtilde.reshape(-1,1), params)
q_NN = variables['Q']
r_NN = variables['r']
...
generateSurfacePlots(mfr_Results, nn_Results, fix_points, X, coor_var, function_name, var_name, plot_content, height=800, width=1300, path = docdir)
...
generateSurfacePlots(mfr_Results, nn_Results, fix_points, X, coor_var, function_name, var_name, plot_content, height=800, width=1000, path = docdir)
